...
bool GCountryInformationWindow::Update()
{
   ...
   GString l_sHumanDevelopmentString = GString::FormatNumber(m_pData->m_fHumanDev, 3) + L", LE " + GString::FormatNumber(m_pData->m_fLifeExpectancy, 1) + L", MYS " + GString::FormatNumber(m_pData->m_fMeanYearsSchooling, 1) + L", EYS " + GString::FormatNumber(m_pData->m_fExpectedYearsSchooling, 1);
   m_pObjCountryLbl->Text( GString(g_ClientDAL.Country(g_ClientDCL.SelectedCountryID()).Code()) +
       L" (" + l_sHumanDevelopmentString + L")");
